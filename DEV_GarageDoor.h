...
struct DEV_GarageDoor : Service::GarageDoorOpener {     // A Garage Door Opener
  ...
  int limitPin = 27;   //use this termporarily 
  int activationPin;
  int warnPin;
  ...
  DEV_GarageDoor(int activationPin, int warnPin) : Service::GarageDoorOpener(){       // constructor() method
    ...
    pinMode(limitPin, OUTPUT);                                    // set the mode for limitPin to be an OUTPUT (std Arduino function)
    ...
    LOG0("- Configuring limitPin at PIN ");
    LOG0(limitPin);
    LOG0("\n");
    digitalWrite(limitPin,HIGH);                   // turn pin for stopLed on
    
    LOG0("- Configuring activationPin at PIN ");
    LOG0(activationPin);
    LOG0("\n");
    digitalWrite(activationPin,HIGH);                   // turn pin for activationLed on
    ...
    digitalWrite(limitPin,LOW);                    // turn pin off
  ...
  boolean update(){                              // update() method
    ...
    if(target->getNewVal()==OPEN){                     // if the target-state value is set to 0, HomeKit is requesting the door to be in open position
      // But if the door was already closing and we get a new signal, door must stopped, not close (logic of the Deimos board)
      ...
      // ¿Tendría quee cambiar el target a 0 para que abra?
        return(false);
      }
  ...
  void loop(){                                     // loop() method
    ...
    if(current->getVal()==CLOSING && random(100000)==0){    // here we simulate a random obstruction, but only if the door is closing (not opening)
    // Habría que cambiar random(100000) por el evento de deteccion de obstaculo de la fotocélula
      current->setVal(STOPPED);                             // if our simulated obstruction is triggered, set the curent-state to 4, which means "stopped"
    ...
    if(current->getVal()==STOPPED){
      //LOG1("Door stopped\n");  
      return;
    }                  // if the current-state is stopped, there is nothing more to do - exit loop()  
    ...
    if(current->getVal()!=target->getVal() && target->timeVal()>5000){ // simulate a garage door that takes 5 seconds to operate by monitoring time since target-state was last modified
      ...
      digitalWrite(limitPin,HIGH);                   // turn pin on
      delay(250);                                     // wait 250 ms
      digitalWrite(limitPin,LOW);                    // turn pin off
      delay(250);                 
      LOG1("Door Inactive \n");
